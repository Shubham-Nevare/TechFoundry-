...
const Hero = ({ 
  ...
  useEffect(() => {
    ...
    if (previewCards) {
      gsap.fromTo(heroRef.current.querySelectorAll('.preview-card'),
        { y: 50, opacity: 0 },
        { 
          y: 0, 
          opacity: 1, 
    ...
    if (stats) {
      gsap.fromTo(heroRef.current.querySelectorAll('.stat-card'),
        { scale: 0, opacity: 0 },
        { 
          scale: 1, 
          opacity: 1, 
    ...
    <section ref={heroRef} className={`relative pt-20 overflow-hidden ${className}`}>
      ...
      <div className="relative max-w-7xl mx-auto mobile-px">
        <div className="text-center mb-8 sm:mb-16">
          ...
          <h1 className="mobile-text-4xl font-bold text-gray-900 mb-4 sm:mb-6 animate-slide-up leading-tight">
            {title}
            {subtitle && (
              <span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent"> {subtitle}</span>
            )}
          </h1>
          ...
          {previewCards && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mobile-gap max-w-5xl mx-auto mb-8 sm:mb-12 animate-fade-in-delay-2">
              {previewCards.map((card, index) => (
                <div key={index} className="preview-card bg-white/80 backdrop-blur-sm rounded-xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-200/50 group">
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 ${card.iconBg} rounded-lg flex items-center justify-center mb-3 sm:mb-4 mx-auto group-hover:scale-110 transition-transform`}>
                    <card.icon className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
                  </div>
                  <h3 className="font-semibold text-gray-900 mb-1 sm:mb-2 mobile-text-sm">{card.title}</h3>
                  <p className="mobile-text-xs text-gray-600">{card.description}</p>
                  {card.price && (
                    <div className={`mobile-text-xs ${card.priceColor} mt-1 sm:mt-2`}>{card.price}</div>
                  )}
                </div>
              ))}
            </div>
          ...
          {stats && (
            <div className="grid grid-cols-2 sm:grid-cols-4 mobile-gap-sm max-w-4xl mx-auto mb-8 sm:mb-12 animate-fade-in-delay-2">
              {stats.map((stat, index) => (
                <div key={index} className="stat-card bg-white/80 backdrop-blur-sm rounded-xl p-3 sm:p-4 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-200/50">
                  <div className={`mobile-text-xl font-bold ${stat.valueColor} mb-0.5 sm:mb-1`}>{stat.value}</div>
                  <div className="mobile-text-xs text-gray-600">{stat.label}</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="hero-buttons flex flex-col sm:flex-row justify-center mobile-gap-sm animate-slide-up-delay">
            <Button size="lg" asChild className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl mobile-text-sm">
              <Link href={primaryButton.href}>{primaryButton.text}</Link>
            </Button>
            <Button size="lg" variant="outline" asChild className="transform hover:scale-105 transition-all duration-300 border-2 mobile-text-sm">
      ...
      {floatingIcons && (
        <div className="absolute inset-0 pointer-events-none mobile-hidden">
          {floatingIcons.map((icon, index) => (
            <div key={index} className={`absolute ${icon.position} animate-float${index > 0 ? `-delay${index}` : ''}`}>
              <div className={`${icon.size} ${icon.bg} rounded-lg flex items-center justify-center backdrop-blur-sm`}>
