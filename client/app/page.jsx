    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      <Hero
        ...
        stats={[
          ...
          { value: '50+', label: 'Happy Clients', valueColor: 'text-green-600' },
          { value: '100%', label: 'Satisfaction', valueColor: 'text-orange-600' }
        ]}
        floatingIcons={[
          { position: 'top-1/4 left-10', size: 'w-12 h-12', bg: 'bg-blue-500/20', component: Globe, iconClass: 'h-6 w-6 text-blue-600' },
          { position: 'top-1/3 right-16', size: 'w-10 h-10', bg: 'bg-purple-500/20', component: Smartphone, iconClass: 'h-5 w-5 text-purple-600' },
          { position: 'bottom-1/3 left-20', size: 'w-14 h-14', bg: 'bg-green-500/20', component: Brain, iconClass: 'h-7 w-7 text-green-600' },
          { position: 'bottom-1/4 right-10', size: 'w-11 h-11', bg: 'bg-orange-500/20', component: Cloud, iconClass: 'h-5 w-5 text-orange-600' }
        ]}
      />
      ...
      <section ref={trustRef} className="mobile-py bg-white border-b">
        <div className="max-w-7xl mx-auto mobile-px">
          <div className="text-center mb-6 sm:mb-8">
            <h2 className="mobile-text-2xl font-bold text-gray-900 mb-2">Trusted & Certified</h2>
            <p className="mobile-text-sm text-gray-600">We maintain the highest standards of quality and security</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 mobile-gap">
            {securityBadges.map((badge, index) => (
              <div key={index} className="trust-badge text-center p-3 sm:p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                <div className="text-2xl sm:text-3xl mb-1 sm:mb-2">{badge.icon}</div>
                <h3 className="font-semibold text-gray-900 mobile-text-xs">{badge.name}</h3>
                <p className="mobile-text-xs text-gray-600">{badge.description}</p>
              </div>
            ))}
          </div>
        </div>
              ...
              return (
                <Card key={service.id} className="service-card group hover:shadow-lg transition-shadow duration-300">
                  <CardHeader className="p-4 sm:p-6">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg flex items-center justify-center mb-3 sm:mb-4">
                      <IconComponent className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
                    </div>
                    <CardTitle className="mobile-text-lg">{service.title}</CardTitle>
                    <CardDescription className="mobile-text-sm">{service.description}</CardDescription>
                  </CardHeader>
                  <CardContent className="p-4 sm:p-6">
                    <div className="flex flex-wrap gap-1 sm:gap-2 mb-3 sm:mb-4">
                      {service.features.slice(0, 3).map((feature, index) => (
                        <Badge key={index} variant="secondary" className="mobile-text-xs">
                          {feature}
                        </Badge>
                      ))}
                    </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mobile-gap">
            {guarantees.map((guarantee, index) => (
              <Card key={index} className="text-center p-4 sm:p-6 hover:shadow-lg transition-shadow">
                ...
                <h3 className="mobile-text-base font-semibold text-gray-900 mb-2">{guarantee.title}</h3>
                <p className="mobile-text-sm text-gray-600">{guarantee.description}</p>
              </Card>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mobile-gap">
            {teamMembers.map((member) => (
                  ...
                  <CardHeader className="p-4 sm:p-6">
                    ...
                    <CardTitle className="mobile-text-base">{member.name}</CardTitle>
                    <CardDescription className="mobile-text-sm">{member.role}</CardDescription>
                  </CardHeader>
                  <CardContent className="p-4 sm:p-6">
                    <p className="mobile-text-sm text-gray-600 mb-3 sm:mb-4">{member.bio}</p>
        ...
        <div className="max-w-7xl mx-auto mobile-px">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mobile-gap">
            {certifications.map((cert, index) => (
              <Card key={index} className="text-center p-4 sm:p-6 hover:shadow-lg transition-shadow">
                <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 bg-white rounded-lg flex items-center justify-center">
                  <img 
                    src={cert.logo} 
                    alt={cert.issuer}
                ...
                <h3 className="font-semibold text-gray-900 mb-1 mobile-text-sm">{cert.name}</h3>
                <p className="mobile-text-xs text-gray-600 mb-2">{cert.issuer}</p>
                <Badge variant="secondary" className="mobile-text-xs">{cert.year}</Badge>
              </Card>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto mobile-px">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mobile-gap">
            {testimonials.map((testimonial) => (
                  ...
                  <p className="mobile-text-sm text-gray-600 mb-3 sm:mb-4">"{testimonial.testimonial}"</p>
                  <div className="space-y-2 sm:space-y-3">
                    <div className="flex items-center space-x-2 sm:space-x-3">
                      ...
                      />
                      <div>
                        <div className="font-semibold text-gray-900 mobile-text-sm">{testimonial.name}</div>
                        <div className="mobile-text-xs text-gray-500">{testimonial.position}</div>
                        <div className="mobile-text-xs text-gray-500">{testimonial.company}</div>
                      </div>
                    </div>
                    <div className="pt-2 sm:pt-3 border-t">
                      <div className="mobile-text-sm font-medium text-gray-900 mb-1 sm:mb-2">Project: {testimonial.project}</div>
                      <div className="space-y-1">
                        {testimonial.metrics.map((metric, index) => (
                          <div key={index} className="flex items-center mobile-text-xs text-gray-600">
      ...
      <section className="mobile-py-xl bg-gradient-to-r from-blue-600 to-purple-600">
        <div className="max-w-7xl mx-auto mobile-px text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center mobile-gap-sm">
            <Button size="lg" variant="secondary" asChild className="mobile-text-sm">
              <Link href="/contact">Start Your Project</Link>
            </Button>
            <Button size="lg" variant="outline" asChild className="border-white text-white bg-transparant hover:bg-white hover:text-purple-600 mobile-text-sm">
